...
import { statusLabel, statusStyle } from '@/lib/event-status'
...
function statusClass(type: SeasonEvent['status']) {
  const style = statusStyle(type)
  if (style === 'red') return styles.statusOpen
  if (style === 'gold') return styles.statusSoon
  return styles.statusGray
}

export default function EventsListing() {
  const { header, months } = eventsListingPage
  const upcoming = upcomingEvents(season.events)
  const pastGroups = groupPastByYear(season.events)
  ...
  return (
    <>
      ...
      <section className={styles.pageHead}>
        <div className={styles.phInner}>
          ...
          {/* Year + months ribbon — highlight tracks the live date,
              pips are generated from upcoming season.events. */}
          <MonthRibbon months={months} events={upcoming} />
        </div>
      </section>
            ...
            return (
              ...
              >
                ...
                <div className={styles.evFoot}>
                  <span className={`${styles.evStatus} ${statusClass(status)}`}>
                    {statusLabel(status)}
                  </span>
      ...
      {/* ── Past archive (derived from retired events) ──── */}
      {pastGroups.length > 0 && (
                ...
                <Reveal key={block.year} className={styles.archYear} delay={0.06 * bi}>
                  ...
                  <div className={styles.archRows}>
                    {block.events.map((ev) => (
                      ...
                      >
                        <div className={styles.archCn}>{ev.titleZh.join('')}</div>
                      </Link>
                    ))}
                  </div>
                </Reveal>
